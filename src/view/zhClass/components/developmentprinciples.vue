<template>
  <div class="app">
    <div class="famousteacher">
      <div class="Etitle">
        <div class="Exhibitiontitle">开 / 发 / 原 / 则</div>
        <div class="introduction">我们一起来推进教育信息化建设</div>
      </div>
      <div class="class">
        <div class="text">
          <div class="title">
            <div class="menu" v-for="(items, index) in list" @mouseenter="mouseenter(index)" :key="index">
              <div class="icon">
                <img class="imgicon" :src="indexd == index?items.iconchoose:items.icon" alt />
              </div>
          ...
          <div class="content">
            ...
            <div class="picture">
              <div class="img">
                <img class="imagebanner" :src="picture" alt />
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getmessage() {
      var that = this;
      Axios.get(
        "https://mock.aarnio.cn/mock/5e4a4a71a7e3066df43697b8/example/zh-classdevelopmentprinciples",
        {
          params: {}, // 参数
          timeout: 3000 // 配置
        }
